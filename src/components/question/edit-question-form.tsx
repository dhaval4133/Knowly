...
import type { QuestionData } from '@/lib/types'; // Use QuestionData for initial values

interface EditQuestionFormProps {
  initialData: Pick<QuestionData, 'title' | 'description' | 'tags'> & { id: string };
}

export default function EditQuestionForm({ initialData }: EditQuestionFormProps) {
  ...
  const handleSuggestTags = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "An error occurred while trying to suggest tags.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('/api/questions/update', {
        ...
        body: JSON.stringify({ questionId: initialData.id, title, description, tags }),
      });
      ...
      if (response.ok && data.success) {
        toast({
          ...
          description: data.message || "Your question has been successfully updated.",
        });
        ...
        toast({
          title: "Update Failed",
          description: data.message || "Could not update your question.",
          variant: "destructive",
        });
  ...
  return (
    <Card className="shadow-lg">
      <form onSubmit={handleSubmit}>
        <CardHeader>
          <CardTitle className="text-2xl md:text-3xl">Edit Your Question</CardTitle>
        </CardHeader>
